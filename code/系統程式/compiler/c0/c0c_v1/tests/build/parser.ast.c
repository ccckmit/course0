...
static unsigned int typedef_hash(const char * s)
{
    unsigned int h = 5381;
    while (*s)
...
{
    unsigned int h = typedef_hash(name);
...
static Token expect(Parser * p, TokenKind k)
{
    Token t = peek(p);
    if (t.kind != k) {
        parse_error(p, t, "expected '%s', got '%s'", token_kind_name(k), token_kind_name(t.kind));
    }
    return advance(p);
}
...
static Node * parse_declarator(Parser * p, Node * base_type, const char * * out_name, int allow_abstract);
...
static int is_type_start(Parser * p)
{
    ...
    case TK_TYPEDEF:
        return 1;
    case TK_IDENT:
        return typedef_contains(&p->typedefs, peek(p).text) || typedef_contains(&p->struct_tags, peek(p).text);
    default:
        return 0;
    }
}
...
static Node * parse_type_spec(Parser * p, Node * base)
{
    ...
    tok = peek(p);
    if (((((((((((flags.is_unsigned || flags.is_signed) && tok.kind != TK_VOID) && tok.kind != TK_CHAR) && tok.kind != TK_SHORT) && tok.kind != TK_INT) && tok.kind != TK_LONG) && tok.kind != TK_FLOAT) && tok.kind != TK_DOUBLE) && tok.kind != TK_STRUCT) && tok.kind != TK_UNION) && tok.kind != TK_ENUM) {
    ...
    case TK_DOUBLE:
        {
            ...
            if (tok.kind == TK_LONG && check(p, TK_LONG)) {
                advance(p);
                n->sval = "long long";
            }
            break;
...
static void parse_param_list(Parser * p, Node * func_type)
{
    ...
    while (!check(p, TK_RPAREN) && !check(p, TK_EOF)) {
        ...
        if ((base->kind == ND_TYPE_BASE && strcmp(base->sval, "void") == 0) && check(p, TK_RPAREN)) {
            break;
        }
    ...
    for (; ; ) {
        ...
        if (!check(p, TK_RPAREN)) {
            ...
            while (!check(p, TK_RPAREN) && !check(p, TK_EOF)) {
                ...
                first = 0;
                call->children = ast_list_append(&p->arena, call->children, parse_assign_expr(p));
            }
        }
        expect(p, TK_RPAREN);
        n = call;
...
static const BinOp BINOPS[] = {{TK_PIPE_PIPE, "||", 4, 0}, {TK_AMP_AMP, "&&", 5, 0}, {TK_PIPE, "|", 6, 0}, {TK_CARET, "^", 7, 0}, {TK_AMP, "&", 8, 0}, {TK_EQ_EQ, "==", 9, 0}, {TK_BANG_EQ, "!=", 9, 0}, {TK_LT, "<", 10, 0}, {TK_GT, ">", 10, 0}, {TK_LT_EQ, "<=", 10, 0}, {TK_GT_EQ, ">=", 10, 0}, {TK_LT_LT, "<<", 11, 0}, {TK_GT_GT, ">>", 11, 0}, {TK_PLUS, "+", 12, 0}, {TK_MINUS, "-", 12, 0}, {TK_STAR, "*", 13, 0}, {TK_SLASH, "/", 13, 0}, {TK_PERCENT, "%", 13, 0}, {0, NULL, 0, 0}};
...
static Node * parse_initializer(Parser * p)
{
    if (check(p, TK_LBRACE)) {
        ...
        while (!check(p, TK_RBRACE) && !check(p, TK_EOF)) {
            ...
            il->children = ast_list_append(&p->arena, il->children, parse_initializer(p));
        }
            if (!match(p, TK_COMMA))
                break;
...
Parser * parser_new_file(const char * filename)
{
    ...
    static const char * std_typedefs[] = {"size_t", "ssize_t", "ptrdiff_t", "intptr_t", "uintptr_t", "uint8_t", "uint16_t", "uint32_t", "uint64_t", "int8_t", "int16_t", "int32_t", "int64_t", "off_t", "pid_t", "uid_t", "gid_t", "FILE", "DIR", "va_list", "__builtin_va_list", "bool", "wchar_t", "wint_t", NULL};
...
int parser_test(const char * filename)
{
    ...
    int count = 0;
    for (NodeList * nl = root->children; nl; nl = nl->next)
        count++;
